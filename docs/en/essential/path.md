...
Each request Elysia will lookup for [route](/en/essential/route) and response using [handler](/en/essential/handler) function.
...
![Representation of URL segments](/en/essential/url-segment.webp)

Path parameters in Elysia are represented by prefixing a segment with ':' follow by a name.
![Representation of path parameter](/en/essential/path-parameter.webp)
