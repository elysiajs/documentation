<template>
    <div
        ref="refElement"
        class="spin-in container-style duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] container relative isolate w-280px sm:w-[480px] transition-transform will-change-transform [aspect-ratio:1/1.586] [contain:layout_style] [perspective:600px]"
    ...
    >
        ...
        >
            ...
            <div
                class="transition-background duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] will-change-background grid size-full opacity-[0.2] mix-blend-soft-light transition-opacity [background:radial-gradient(farthest-corner_circle_at_var(--m-x)_var(--m-y),_rgba(255,255,255,0.8)_10%,_rgba(255,255,255,0.65)_20%,_rgba(255,255,255,0)_90%)] [clip-path:inset(0_0_1px_0_round_var(--radius))] [grid-area:1/1]"
            />
            <div
...
.spin-in {
	/* animation: spin-in 1.2s cubic-bezier(0.34, 1.56, 0.64, 1); */
}

@keyframes spin-in {
	0% {
		filter: brightness(1.1) saturate(150%) blur(16px);
		transform: translateY(100%) rotateX(15deg) rotateY(135deg) rotateZ(15deg);
	}
	100% {
